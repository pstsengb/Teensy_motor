...
void calculateXY(){
  ...
  displacement = ((m1.target_rpm_l/30*3.14159*0.065/4/60)/2+(m1.target_rpm_r/30*3.14159*0.065/4/60)/2) *(m1.dt); 
  rad = rad + ((m1.target_rpm_l/30*3.14159*0.065/4/60/ 0.081)/2 + (-m1.target_rpm_r/30*3.14159*0.065/4/60/ 0.081)/2) *(m1.dt); 
  x_pos = x_pos + displacement*cos(rad);
  y_pos = y_pos + displacement*sin(rad);
}
...
void loop() {
  ...
  odo_type.header.frame_id = odom;
  odo_type.child_frame_id = base_link;
  odo_type.pose.pose.position.x = x_pos;
  odo_type.pose.pose.position.y = y_pos;
  odo_type.pose.pose.orientation = tf::createQuaternionFromYaw(rad);
  odo_type.twist.twist.linear.x = estimateV;
  odo_type.twist.twist.angular.z = estimateW ;
  odo_type.header.stamp = nh.now();
  ...
  t.transform.rotation = tf::createQuaternionFromYaw(rad);
  t.header.stamp = odo_type.header.stamp; 
